...
export const analyzeMealWithGemini = async (
  ...
  name: string;
  nutrition: { calories: number; protein: number; fat: number; carbs: number; estimatedWeight: string };
}> => {
  try {
    const parts: any[] = [];
    ...
    if (textDescription) {
      parts.push({
        text: `Analyze this meal description/image. Identify the food and provide a detailed nutritional breakdown properly estimating calories, protein, fats, and carbs. Description: "${textDescription}"`
      });
    } else if (imageBase64) {
      parts.push({
        text: "Analyze this food image. Identify the food and provide a detailed nutritional breakdown properly estimating calories, protein, fats, and carbs."
      });
    }
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        systemInstruction: "You are an expert nutritionist. Be conservative but realistic with calorie and macro estimates. Ensure you provide estimates for Protein, Carbs, and Fats. Provide a single object response."
      },
    });
